		...
		LRESULT CALLBACK WindowProc(HWND iwindow_handler, UINT imessage, WPARAM iwparam, LPARAM ilparam) {
			...
			if(window) {
				...
				case WM_LBUTTONDOWN:
					...
					window->eventhub().onButtonPress(input::Key(input::Key::MouseLeft), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
					window->cursor().select("hand.click");
					return 0;

				case WM_LBUTTONUP:
					window->eventhub().onButtonRelease(input::Key(input::Key::MouseLeft), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
					window->cursor().select("hand.point");
					return 0;

				case WM_RBUTTONDOWN:
					window->eventhub().onButtonPress(input::Key(input::Key::MouseRight), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
					return 0;

				case WM_RBUTTONUP:
					window->eventhub().onButtonRelease(input::Key(input::Key::MouseRight), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
					return 0;

				case WM_MBUTTONDOWN:
					window->eventhub().onButtonPress(input::Key(input::Key::MouseMiddle), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
					return 0;

				case WM_MBUTTONUP:
					window->eventhub().onButtonRelease(input::Key(input::Key::MouseMiddle), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
					return 0;

				case WM_XBUTTONDOWN: {
						if(HIWORD(iwparam) == XBUTTON1) {
							window->eventhub().onButtonPress(input::Key(input::Key::MouseX1), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
						} else if(HIWORD(iwparam) == XBUTTON2) {
							window->eventhub().onButtonPress(input::Key(input::Key::MouseX2), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
						}
						return 0;
					}
				...
				case WM_XBUTTONUP: {
						if(HIWORD(iwparam) == XBUTTON1) {
							window->eventhub().onButtonRelease(input::Key(input::Key::MouseX1), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
						} else if(HIWORD(iwparam) == XBUTTON2) {
							window->eventhub().onButtonRelease(input::Key(input::Key::MouseX2), GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
						}
						return 0;
					}

				case WM_MOUSEMOVE:
					window->eventhub().onMouseMove(GET_X_LPARAM(ilparam), GET_Y_LPARAM(ilparam));
					return 0;

				case WM_MOUSEWHEEL:
					window->eventhub().onWheelMove(((short)HIWORD(iwparam))/(float)WHEEL_DELTA, 0.0f);
					return 0;

				case WM_MOUSEHWHEEL:
					window->eventhub().onWheelMove(0.0f, ((short)HIWORD(iwparam))/(float)WHEEL_DELTA);
					return 0;
