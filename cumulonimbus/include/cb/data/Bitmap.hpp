	...
	namespace data {
		namespace bmp {
			...
			struct tLA {
				...
				tvec2<T, P> vec() const {return tvec2<T, P>(l, a);}
				tLA &operator=(const tvec2<T, P> &ivec) {l = ivec.r; a = ivec.g; return *this;}
			};
			...
			struct tRGB {
				...
				tvec3<T, P> vec() const {return tvec3<T, P>(r, g, b);}
				tRGB &operator=(const tvec3<T, P> &ivec) {r = ivec.r; g = ivec.g; b = ivec.b; return *this;}
			};
			...
			struct tRGBA {
				...
				tvec4<T, P> vec() const {return tvec4<T, P>(r, g, b, a);}
				tRGBA &operator=(const tvec4<T, P> &ivec) {r = ivec.r; g = ivec.g; b = ivec.b; a = ivec.a; return *this;}
			};
		...
		public:
			...
			Bitmap(const Blob &ifile, bmp::Format iformat, bmp::Type itype);
			...
			void load(const Blob &ifile, bmp::Format iformat, bmp::Type itype);
			void load(const Bitmap &iimg);

			void loadPage(const Blob &ifile, uint32 iz);
		...
		inline Bitmap::Bitmap(const Blob &ifile, bmp::Format iformat, bmp::Type itype)
		: _width(0), _height(0), _depth(0), _pitch(0), _bytes_per_pixel(0), _format(bmp::Format::Void), _type(bmp::Type::Void), _data(nullptr) {
		...
		template<typename T, bmp::Format f, bmp::Type t>
		inline BitmapBase<T,f,t>::BitmapBase(const Blob &ifile)
		: _bitmap(ifile, f, t) {
		}
		...
		template<typename T, bmp::Format f, bmp::Type t> inline void BitmapBase<T,f,t>::load(const Blob &ifile) {_bitmap.load(ifile, f, t);}
